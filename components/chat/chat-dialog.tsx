...
export function ChatDialog({ open, onClose, locale, theme }: ChatDialogProps) {
  ...
  const showHeader = !(isMobile && inputFocused)
  return (
    <AnimatePresence>
      {open ? (
            ...
            >
                  ...
                  <div className="flex w-full items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-[72px] w-[72px] shrink-0 overflow-hidden rounded-full">
                        <Image src="/Chat_avatar.png" alt="Max Webberater" width={72} height={72} className="h-full w-full object-cover" />
                      </div>
                    ...
                    <button
                      type="button"
                      onClick={onClose}
                      aria-label={locale === 'de' ? 'Chat schließen' : 'Закрыть чат'}
              ...
              <div className="min-h-0 flex-1 overflow-hidden">
                <ChatWindow
                  ...
                  theme={theme}
                  autoFocus={!isMobile}
                  onInputFocusChange={(focused) => setInputFocused(focused)}
                />
              </div>
            </div>
          </div>
