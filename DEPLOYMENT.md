...
| Секрет       | Значение                                      |
|--------------|-----------------------------------------------|
| `VM_SSH_KEY` | **Приватный** SSH-ключ — содержимое `~/.ssh/ocivm_key` (без .pub) |
| `VM_USER`    | `opc`                                         |
| `VM_HOST`    | `79.76.120.221`                               |

Без `VM_SSH_KEY` деплой не запустится.

**Важно:** Ключ должен быть без passphrase. Если `error in libcrypto` — пересоздай ключ: `ssh-keygen -t ed25519 -f ~/.ssh/ocivm_key -N ""` и добавь публичный ключ на сервер.

**Connection reset by peer:** Если SSH обрывается — Oracle Cloud может блокировать IP GitHub. Проверь Security List: порт 22 должен быть открыт для `0.0.0.0/0`. Workflow делает 5 попыток с паузой 10 сек.
